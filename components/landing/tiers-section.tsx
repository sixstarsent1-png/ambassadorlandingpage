...
import { Users, GraduationCap, Building2, Camera, MapPinned, Crown, Star } from "lucide-react"
      ...
      <div className="container mx-auto px-4 relative z-10" ref={ref}>
        ...
        >
          ...
          >
            ...
            <span className="text-xs font-bold uppercase tracking-widest text-gold">Ambassador Tiers</span>
          </motion.div>
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-black text-balance">
            Find your <span className="text-gold">place</span> in the movement
          </h2>
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {tiers.map((tier, index) => (
            <motion.div
              ...
              } ${tier.featured ? "ring-2 ring-gold/50" : ""}`}
            >
              {/* Badge */}
              <div className="flex items-start justify-between mb-4 relative z-10">
                <motion.div 
                  ...
                  transition={{ duration: 0.5 }}
                  className="w-14 h-14 rounded-2xl bg-gold/20 flex items-center justify-center group-hover:scale-110 transition-transform duration-300"
                >
                  <tier.icon className="w-7 h-7 text-gold" />
                </motion.div>
                <span className="text-xs font-bold uppercase tracking-wider px-3 py-1 rounded-full bg-gold/20 text-gold">
                  {tier.badge}
                </span>
              </div>
              
              <div className="relative z-10">
                <h3 className="font-black text-xl mb-1 text-foreground group-hover:text-gold transition-colors">{tier.name}</h3>
